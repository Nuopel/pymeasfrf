...
def record_1_mic(channel2record, record_time, fs, path2save_record = './record', name_record = '/record_mic_', opt_plot = 1, opt_save = 1):
...
#% REcord FRF
def record_frf_1spk_2_1mic(mic2record,spk2record,sig2play, gain,FS,n_avg = 1, path2save_record= './record/', opt_plot = 1, opt_save = 1, filename_recTF = [] ):
    ...
    signal_in = record_frf_1spk_2_allmics(spk2record=spk2record,sig2play=sig2play, gain = gain,n_avg=n_avg, FS = FS, opt_plot = opt_plot, opt_save = 0 )
        ...
        if len(filename_recTF) == 0 : 
            ...
            if n_avg > 1 :
                for i_avg in range(n_avg):
                    filename_recTF =  '/LS' + str(spk2record) + 'Mic' + str(mic2record) + '_avg%iover%i'%(i_avg+1,n_avg) + '.wav'
                    wavfile.write(path2save_record+ filename_recTF, int(FS), (signal_in[:,:,i_avg]).astype(np.float32))
            else :
...
def record_frf_1spk_2_allmics(spk2record,sig2play, gain,FS,n_avg = 1, path2save_record= './record', opt_plot = 1, opt_save = 1 ):
...
def record_tf_allspeakers_2_allmics(sig2play, gain,FS,path2save_record,n_avg = 1 ,mic_array_pos=0, opt_plot = 1, opt_save = 1 ):
    ...
    for out_ch in range(N_SPK):
        signal_in = record_frf_1spk_2_allmics(spk2record = out_ch,sig2play=sig2play, gain = gain,n_avg = n_avg, FS=FS, path2save_record = path2save_record, opt_plot = opt_plot, opt_save = 0 )
        
        if opt_save == 1:
            if n_avg == 1:
                for m in range(M_MIC): # write the measurements to the wav file
                    filename_recTF = path2save_record + '/LS' + str(out_ch) + 'Mic' + str(m+mic_array_pos*M_MIC) + '.wav'
                    wavfile.write(filename_recTF, int(FS), (signal_in[::, m]).astype(np.float32))
            else :
                for m in range(M_MIC): # write the measurements to the wav file
                    for i_avg in range(n_avg):
                        filename_recTF = path2save_record + '/LS' + str(out_ch) + 'Mic' + str(m+mic_array_pos*M_MIC)+ 'avg_%iover%i'%(i_avg,n_avg) + '.wav'
...
def record_frf_1spk_2_1mic_latency_measurement_short_signal_input(mic2record,spk2record,latency_feedback_mic,latency_feedback_spk,
    ...
    signal_mics = record_frf_1spk_2_1mic_latency_measurement(mic2record,spk2record,latency_feedback_mic,latency_feedback_spk,
...
def record_frf_1spk_2_1mic_latency_measurement(mic2record,spk2record,latency_feedback_mic,latency_feedback_spk,
    ...
    #record
    signal_mics = record_frf_1spk_2_allmics(spk2record_tot, input_sig, gain=gain_spk,n_avg = n_avg, FS=fs, path2save_record= './record1/', opt_plot = opt_plot, opt_save = 0 )
...
def record_frf_multioutput_allmics_latency_measurement(mic2record,latency_feedback_mic,latency_feedback_spk,
    ...
    burst, t_tot,t_burst = gen_burst(fs, t_tot = t_tot, t_burst = t_burst, gain = gain_burst ) # gen burst
    shape2add= np.array(sig2play.shape);    shape2add[0] = 1

    reshape2add = np.ones(len(sig2play.shape),dtype = int);reshape2add[0] = burst.shape[0]
